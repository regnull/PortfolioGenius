...
class TiingoCryptoPriceTool(BaseTool):
    ...
    def _run(self, symbol: str, start_date: Optional[str] = None, end_date: Optional[str] = None, frequency: str = "daily") -> str:
        try:
            ...
            # Get historical data
            history_url = f"https://api.tiingo.com/tiingo/crypto/prices"
            ...
            if top_data and len(top_data) > 0:
                top_book = top_data[0].get("topOfBookData", [])
                if top_book:
                    current_price = top_book[0].get("lastPrice", "N/A")
            ...
            result = {
                ...
                "start_date": start_date,
                "end_date": end_date,
                "data_points": len(history_data[0].get("priceData", [])) if history_data else 0,
                "latest_data": history_data[0].get("priceData", [])[-5:] if history_data and history_data[0].get("priceData") else [],
                "retrieved_at": datetime.now().strftime("%Y-%m-%d %H:%M:%S")
            }
